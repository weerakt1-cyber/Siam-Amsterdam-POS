...
export async function POST(req: NextRequest) {
  try {
    ...
    const title = typeof body.title === 'string' ? body.title.trim().slice(0, 40) : undefined
    if (!name?.trim()) return NextResponse.json({ error: 'Name required' }, { status: 400 })
    if (!pin || !/^\d{4}$/.test(String(pin))) return NextResponse.json({ error: 'PIN must be exactly 4 digits' }, { status: 400 })
    // `role` is the access LEVEL (drives permissions); `title` is the free-text position.
    if (!['admin', 'manager', 'bartender', 'staff'].includes(role)) return NextResponse.json({ error: 'Invalid role' }, { status: 400 })
    const user = await createStaffMember({ name: String(name).trim(), role, title, pin: String(pin), color: color ?? '#10b981' }, storeId)
