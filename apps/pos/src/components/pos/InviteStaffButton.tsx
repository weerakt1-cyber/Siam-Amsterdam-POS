        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onPointerDown={() => setOpen(false)}>
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-2xl" onPointerDown={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-black text-gray-900">{L('Invite staff', 'เชิญพนักงาน')}</h3>
              <button onPointerDown={() => setOpen(false)} className="text-gray-400 text-lg leading-none px-1">✕</button>
            </div>
            <p className="text-xs text-gray-500 mb-4 leading-relaxed">
              {L('Send this link for staff to open on their own device — after signing up they auto-join this store', 'ส่งลิงก์นี้ให้พนักงานเปิดในเครื่องของตัวเอง — สมัครบัญชีแล้วจะเข้าร่วมร้านนี้โดยอัตโนมัติ')}
            </p>

            {loading ? (
              <p className="text-sm text-gray-400">{L('Loading…', 'กำลังโหลด…')}</p>
            ) : token ? (
              <>
                <div className="flex gap-2">
                  ...
                  <button onPointerDown={copy}
                    ...
                    {copied ? L('Copied ✓', 'คัดลอกแล้ว ✓') : L('Copy', 'คัดลอก')}
                  </button>
                </div>
                <button onPointerDown={generate} disabled={busy}
                  className="mt-3 text-[11px] text-red-500 font-semibold disabled:opacity-40">
                  {busy ? L('Creating…', 'กำลังสร้าง…') : L('New link (revokes all old links)', 'สร้างลิงก์ใหม่ (ยกเลิกลิงก์เดิมทั้งหมด)')}
                </button>
              </>
            ) : (
              <button onPointerDown={generate} disabled={busy}
                ...
                {busy ? L('Creating…', 'กำลังสร้าง…') : L('Create invite link', 'สร้างลิงก์เชิญ')}
              </button>
            )}
          </div>
        </div>
