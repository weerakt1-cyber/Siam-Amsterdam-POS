...
function readBarSettings(): { barName: string; address: string } {
  try {
    ...
    return {
      barName: obj.barName ?? 'Your Bar',
      address: obj.address ?? '',
    }
  } catch {
    return { barName: 'Your Bar', address: '' }
  }
}
...
export function generateDailyReportPDF(
  ...
  doc.text('Generated by PLOEN POS', MAR, PH - 9)
  ...
  const fileSlug = settings.barName.replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-+|-+$/g, '').toUpperCase() || 'BAR'
  doc.save(`${fileSlug}-report-${date}.pdf`)
}
