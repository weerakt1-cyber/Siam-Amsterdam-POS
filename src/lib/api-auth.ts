...
// ─── Platform super-admin (operator) ──────────────────────────────────────────
// The one (or few) accounts that manage subscriptions across ALL stores — a
// different axis from the per-store `role`. Identified by an email allow-list in
// SUPER_ADMIN_EMAILS (comma-separated). The email comes from the verified JWT,
