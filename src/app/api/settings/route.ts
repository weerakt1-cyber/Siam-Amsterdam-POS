...
export async function GET() {
  try {
    const cfg = await getConfigMany([K_BAR, K_FLOOR])
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.barSettings !== undefined) await setConfig(K_BAR, JSON.stringify(body.barSettings))
    if (body.floorTiles  !== undefined) await setConfig(K_FLOOR, JSON.stringify(body.floorTiles))
